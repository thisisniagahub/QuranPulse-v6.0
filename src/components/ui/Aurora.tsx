...
/**
 * Aurora Background — WebGL version from ReactBits
 * Premium animated aurora light effect using OGL (WebGL).
 * Default colors set to QuranPulse Raudhah theme (teal/gold).
 */
...
#define COLOR_RAMP(colors, factor, finalColor) { \\
  int index = 0; \\
  for (int i = 0; i < 2; i++) { \\
    index = colors[i].position <= factor ? i : index; \\
  } \\
  ColorStop currentColor = colors[index]; \\
  ColorStop nextColor = colors[min(index + 1, 2)]; \\
...
void main() {
  ...
  ColorStop cycleColors[3];
  cycleColors[0] = ColorStop(uColorStops[0], 0.0);
  cycleColors[1] = ColorStop(uColorStops[1], 0.5);
  cycleColors[2] = ColorStop(uColorStops[2], 1.0);
  
  vec3 rampColor;
  COLOR_RAMP(cycleColors, uv.x, rampColor);
...
export const Aurora: React.FC<AuroraProps> = ({
    ...
    useEffect(() => {
        ...
        try {
            ...
            // WebGL not available, fallback to empty
            return;
        }
        ...
        function resize() {
            ...
            if (program) (program.uniforms as any).uResolution.value = [width, height];
        }
        ...
        const update = (t: number) => {
            ...
            if (program) {
                (program.uniforms as any).uTime.value = t * 0.01 * s * 0.1;
