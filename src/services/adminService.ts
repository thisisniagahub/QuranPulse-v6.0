...
export const adminService = {
  ...
  updateUser: async (userId: string, updates: { full_name?: string; email?: string; role?: string; tier?: string }) => {
  ...
  banUser: async (userId: string, reason: string) => {
    // In a real app, you might have a dedicated 'bans' table or a status field
    // For now assuming 'status' column exists or using metadata
    const { error } = await supabase
      .from('profiles')
      .update({ status: 'BANNED' }) // Ensure this column exists in your schema!
      .eq('id', userId);
  ...
  // Update User Tier
  updateUserTier: async (userId: string, tier: 'FREE' | 'PRO' | 'FAMILY') => {
     const { error } = await supabase
      .from('profiles')
      .update({ tier })
      .eq('id', userId);
