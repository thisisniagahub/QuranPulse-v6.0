...
const IqraGameEngine: React.FC = () => {
    ...
    if (loading) return <div className="flex items-center justify-center h-64 text-raudhah-teal animate-pulse">Initializing IQRA Engine...</div>;

    if (showLevelUp) return (
        <motion.div
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="flex flex-col items-center justify-center p-12 bg-black/40 border border-raudhah-teal/20 rounded-3xl backdrop-blur-xl"
        >
