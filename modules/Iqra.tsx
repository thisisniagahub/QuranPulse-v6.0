...
import { analyzeQuranRecitation, generateIslamicVideo, generateSpeech } from '../services/geminiService';
...
const Iqra: React.FC<IqraProps> = ({ user, onUpdateUser }) => {
  // Mode State
  const [mode, setMode] = useState<'READ' | 'COACH' | 'TUTORIALS'>('READ');
        ...
        return (
            <div className="h-full flex flex-col items-center justify-center text-slate-500 animate-fade-in bg-slate-900 rounded-2xl border border-slate-800 p-6">
                ...
                <button 
                    ...
                    className="px-6 py-3 bg-teal-600 hover:bg-teal-500 text-white rounded-xl font-bold transition-colors"
                >
    ...
    <div className="h-full flex flex-col space-y-4 relative animate-fade-in">
        ...
        <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800 space-y-3 shadow-lg z-10">
                ...
                <div className="flex gap-2">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {pageProgress && (
                                <span className="px-2 py-0.5 rounded bg-green-500/20 text-green-400 text-[10px] font-bold border border-green-500/20">
            ...
            <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                <div 
                    className="h-full bg-gradient-to-r from-teal-pulse to-teal-500 transition-all duration-700 ease-out"
            ...
            <div className="flex justify-between items-center text-[10px] uppercase font-bold tracking-wider text-slate-500">
                ...
                {bookPercentage >= 100 && (
                    <span className="text-gold-400 flex items-center gap-1 animate-pulse">
            ...
            {bookPercentage >= 100 && (
                <div className="bg-gold-500/10 border border-gold-500/30 p-2 rounded-lg flex items-center justify-between">
                     <div className="flex items-center gap-2">
                         <i className="fa-solid fa-award text-gold-400 text-xl"></i>
                         <div>
                             <p className="text-white text-xs font-bold">Iqra 1 Certificate</p>
                             <p className="text-[10px] text-slate-400">Ready to download</p>
                         </div>
                     </div>
                     <button className="text-xs bg-gold-500 text-black px-3 py-1 rounded font-bold hover:bg-gold-400">View</button>
                </div>
            )}
        </div>
        ...
        <div key={currentPageIndex} className="flex-1 bg-[#fffdf5] rounded-xl shadow-2xl overflow-hidden relative text-black p-2 sm:p-6 border-l-8 border-l-slate-800 border-r-2 border-r-slate-300 animate-slide-up origin-bottom min-h-[400px]">
            ...
            {/* Completed Stamp */}
            {pageProgress && (
                <div className="absolute top-4 right-4 z-20 rotate-12 border-4 border-green-600 text-green-600 rounded-lg px-4 py-2 font-bold text-xl opacity-0 animate-[fadeIn_0.5s_ease-out_0.5s_forwards] font-serif">
                    LULUS
                </div>
            )}
                    ...
                    <div key={rowIndex} className="flex border-b-2 border-black last:border-b-0 relative">
                        ...
                        {row.map((cell, colIndex) => {
                            ...
                            return (
                                <div 
                                    key={colIndex} 
                                    className={`flex-1 h-24 sm:h-32 flex items-center justify-center border-l-2 border-black last:border-l-0 hover:bg-yellow-100 cursor-pointer transition-colors duration-200 relative group ${activePlayingCell === cellId ? 'bg-yellow-200' : ''}`}
                                    onClick={() => playGeminiTTS(cell, 'Zephyr', cellId)}
                                >
                                    ...
                                    <span className="absolute bottom-1 text-[8px] text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity">Tap to listen</span>
                                </div>
                            );
                        })}
                    </div>
        ...
        <div className="flex gap-3 pb-safe">
            <button 
                ...
                className="flex-1 bg-teal-pulse hover:bg-teal-400 text-black font-bold py-4 rounded-2xl shadow-lg shadow-teal-500/20 flex items-center justify-center gap-2 transition-all transform active:scale-95 hover:-translate-y-1"
            >
  ...
  const renderVoiceCoach = () => {
    ...
    <div className="h-full flex flex-col items-center justify-center space-y-8 relative animate-fade-in">
      ...
      <div className="bg-slate-800/50 p-6 rounded-3xl border border-slate-700 text-center max-w-sm w-full backdrop-blur-sm shadow-xl">
        ...
        <div className="font-arabic text-3xl text-teal-pulse my-4 leading-relaxed drop-shadow-md" dir="rtl">
          ...
          <div className="flex justify-center items-center gap-1.5 h-16 w-full max-w-[200px]">
              {[...Array(5)].map((_, i) => (
                  <div 
                    ...
                    style={{ 
                        height: `${Math.max(10, Math.min(100, volume * (1 + i/2)))}%`, // Scale based on real volume
      ...
      <div className="relative group flex items-center justify-center">
        ...
        >
          {isRecording ? (
             ...
             <div className="w-24 h-24 rounded-full bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center text-white shadow-lg shadow-teal-500/40 hover:scale-105 transition-transform">
        ...
        <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-3xl p-6 animate-slide-up shadow-xl transition-all duration-500">
            ...
            <div className="space-y-3">
                {result.accuracy_score > 70 && (
                    <div className="bg-gold-500/10 p-3 rounded-xl border border-gold-500/30 mb-2 flex items-center gap-3 animate-fade-in">
                        <div className="w-8 h-8 rounded-full bg-gold-500 text-black flex items-center justify-center font-bold">
                            <i className="fa-solid fa-trophy"></i>
                        </div>
                        <div>
                            <p className="text-gold-400 font-bold text-sm">Page Completed!</p>
                            <p className="text-xs text-gold-200/70">Progress Saved & XP Earned</p>
                        </div>
                    </div>
                )}
                ...
                {result.tajweed_errors.length > 0 ? (
                    <div className="bg-red-900/20 p-3 rounded-xl border border-red-500/20 animate-fade-in">
                        ...
                        <ul className="space-y-2">
                            {result.tajweed_errors.map((item, i) => (
                                <li 
                                    key={i} 
                                    className="bg-black/20 p-2 rounded-lg border border-white/5 animate-slide-up"
                                ...
                                >
                                    <div className="flex justify-between items-start">
                                        ...
                                        <button 
                                          ...
                                          className="text-[10px] text-teal-400 bg-teal-900/30 px-2 py-0.5 rounded hover:bg-teal-900/50 transition-colors"
                                        >
                                          <i className="fa-solid fa-volume-high"></i>
                                        </button>
                                    </div>
                                    <p className="text-[10px] text-teal-400 flex items-start gap-1">
                    ...
                    <div className="bg-green-900/20 p-3 rounded-xl border border-green-500/20 flex items-center gap-2 animate-fade-in">
                        <i className="fa-solid fa-check-circle text-green-400"></i>
                        <p className="text-xs text-green-300">Excellent recitation! No major errors detected.</p>
                    </div>
                )}
  ...
  const renderTutorials = () => (
      <div className="space-y-4 animate-slide-up h-full overflow-y-auto">
          ...
          <div className="flex items-center gap-3 mb-6">
              <button onClick={() => setMode('READ')} className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 hover:text-white">
