...
import { generateIslamicVideo, enhanceVideoPrompt } from '../services/geminiService';
    ...
    <div className="p-4 pb-24 space-y-6 animate-fade-in min-h-full flex flex-col">
      ...
      {activeTab === 'LISTEN' && (
          <div className="flex-1 overflow-y-auto space-y-6 animate-slide-up">
              ...
              <div className="bg-gradient-to-r from-violet-900 to-fuchsia-900 rounded-2xl p-6 flex items-center gap-4 relative overflow-hidden">
                  ...
                  <button className="w-12 h-12 rounded-full bg-white text-fuchsia-900 flex items-center justify-center shadow-lg hover:scale-105 transition-transform z-10">
                      ...
                      {[
                          ...
                          <div key={i} className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-800 transition-colors cursor-pointer group border border-transparent hover:border-slate-700">
                              ...
                              <div className="flex-1">
                                  <h5 className="text-sm font-bold text-white group-hover:text-cyan-400 transition-colors">{pod.title}</h5>
                                  <p className="text-xs text-slate-500">{pod.author} • {pod.time}</p>
                              </div>
                              <button className="text-slate-500 hover:text-white"><i className="fa-regular fa-circle-play text-xl"></i></button>
                          </div>
                      ))}
                  </div>
              </div>
      ...
      {activeTab === 'CREATE' && (
        <div className="animate-slide-up space-y-4">
            <div className="bg-slate-900/80 backdrop-blur border border-slate-700 rounded-2xl p-1 relative group focus-within:border-cyan-500/50 transition-colors">
                {/* Text Area */}
                <textarea
                value={prompt}
                ...
                <button 
                  ...
                  className="absolute bottom-16 right-4 z-20 text-xs bg-slate-800/80 backdrop-blur text-purple-400 border border-purple-500/30 px-3 py-1.5 rounded-lg hover:bg-purple-900/20 hover:text-white transition-all flex items-center gap-2"
                >
                ...
                <button
                    ...
                    className={`px-5 py-2 rounded-xl text-sm font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-lg ${
                        ...
                        : 'bg-gradient-to-r from-purple-600 to-cyan-600 hover:from-purple-500 hover:to-cyan-500 text-white shadow-cyan-500/20 hover:shadow-cyan-500/40'
                    }`}
                >
                    {isGenerating ? (
                    <>
                ...
                <div className="text-center py-12 space-y-6 animate-fade-in">
                    <div className="relative w-32 h-32 mx-auto">
                        ...
                        <div className="w-20 h-20 bg-cyan-500/10 rounded-full flex items-center justify-center animate-pulse">
                            <i className="fa-solid fa-film text-3xl text-cyan-400"></i>
                        </div>
                        </div>
                    </div>
            ...
            {videoUrl && (
                ...
                <div className="rounded-2xl overflow-hidden border border-slate-700 shadow-[0_0_30px_rgba(34,211,238,0.2)] bg-black aspect-video relative group">
                    <video 
                        src={videoUrl} 
                        controls 
                        autoPlay 
