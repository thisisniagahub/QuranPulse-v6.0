...
const Ibadah: React.FC = () => {
  ...
  return (
    <div className="p-4 pb-24 h-full animate-fade-in flex flex-col">
       ...
       <div className="flex bg-slate-900 p-1.5 rounded-2xl mb-8 border border-slate-800 shadow-inner overflow-x-auto no-scrollbar shrink-0">
           {['TASBIH', 'QIBLA', 'PRAYER', 'NOTIFY'].map(tab => (
               ...
               >
                   {activeTab === tab && (
                       <div className="absolute inset-0 bg-gradient-to-r from-teal-500 to-teal-700 rounded-xl"></div>
                   )}
       ...
       {activeTab === 'TASBIH' && (
           <div className="flex-1 flex flex-col items-center justify-center space-y-12 animate-slide-up">
               ...
               <div className="relative group select-none">
                   <div className="absolute inset-0 bg-teal-500 blur-[60px] opacity-20 group-hover:opacity-30 transition-opacity rounded-full"></div>
                   <div className="w-72 h-72 rounded-full bg-slate-950 shadow-[0_20px_50px_-10px_rgba(0,0,0,0.8)] border border-slate-800 flex items-center justify-center p-4">
                       <div 
                         onClick={handleCount}
                         className={`
                            w-full h-full rounded-full bg-gradient-to-b from-slate-800 to-slate-900 
                       ...
                       >
                           ...
                           <span className="text-8xl font-mono text-transparent bg-clip-text bg-gradient-to-b from-teal-300 to-teal-600 font-bold drop-shadow-sm select-none">
                               {count}
                           </span>
               ...
               <div className="flex gap-4">
                   <button onClick={handleReset} className="w-14 h-14 rounded-2xl bg-slate-800 text-red-400 flex items-center justify-center hover:bg-slate-700 hover:text-red-300 border-b-4 border-slate-900 active:border-b-0 active:translate-y-1 transition-all">
                   ...
                   <button onClick={() => setTarget(target === 33 ? 99 : 33)} className="px-6 h-14 rounded-2xl bg-slate-800 text-white font-bold hover:bg-slate-700 text-sm border-b-4 border-slate-900 active:border-b-0 active:translate-y-1 transition-all flex items-center gap-2">
                       <i className="fa-solid fa-bullseye text-teal-400"></i>
                       Target: {target}
                   </button>
               </div>
           </div>
           ...
           <div className="flex-1 flex flex-col items-center justify-center space-y-8 animate-slide-up">
               {!permissionGranted ? (
                   <div className="text-center p-8 bg-slate-800/50 rounded-3xl border border-slate-700">
                       <div className="w-20 h-20 bg-teal-900/30 rounded-full flex items-center justify-center mx-auto mb-6 text-teal-400 animate-pulse">
                       ...
                       <button onClick={startCompass} className="px-8 py-3 bg-teal-500 hover:bg-teal-400 text-black font-bold rounded-xl shadow-lg shadow-teal-500/20 transition-all">Allow Access</button>
                   </div>
               ) : (
                   <>
                        <div className="relative w-80 h-80">
                             <div className="absolute inset-0 bg-teal-500 blur-[80px] opacity-10 rounded-full"></div>
                             <div className="w-full h-full rounded-full bg-gradient-to-br from-slate-800 to-slate-950 border-8 border-slate-800 shadow-[0_20px_50px_-10px_rgba(0,0,0,0.5)] relative transition-transform duration-500 ease-out flex items-center justify-center overflow-hidden" style={{ transform: `rotate(${-heading}deg)` }}>
                                 ...
                                 <div className="absolute top-1/2 left-1/2 w-1 h-32 origin-bottom z-10" style={{ transform: `translate(-50%, -100%) rotate(${QIBLA_OFFSET}deg)` }}>
                                     <div className="absolute -top-6 left-1/2 -translate-x-1/2 flex flex-col items-center">
                                         <span className="text-2xl drop-shadow-lg filter">🕋</span>
                                         <div className="w-1 h-20 bg-gradient-to-t from-transparent to-gold-400 rounded-full"></div>
                                     </div>
                                 </div>
                             </div>
                             <div className="absolute inset-0 rounded-full bg-gradient-to-br from-white/10 to-transparent pointer-events-none border border-white/10"></div>
                             <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-gold-500 text-4xl z-20 drop-shadow-[0_0_10px_rgba(245,158,11,0.5)]"><i className="fa-solid fa-caret-down"></i></div>
                        </div>
       ...
       {activeTab === 'PRAYER' && prayerData && (
           <div className="space-y-4 animate-slide-up overflow-y-auto">
               <div className="relative overflow-hidden bg-gradient-to-r from-teal-900 to-slate-900 p-8 rounded-3xl text-center shadow-lg border border-white/5">
                   ...
                   <div className="flex items-center justify-center gap-2 text-teal-300 mt-2 relative z-10">
               ...
               <div className="bg-slate-900 rounded-3xl p-2 border border-slate-800">
                   {Object.entries(prayerData).filter(([k]) => ['Fajr', 'Sunrise', 'Dhuhr', 'Asr', 'Maghrib', 'Isha'].includes(k)).map(([name, time], idx) => (
                       <div key={name} className="flex justify-between items-center p-4 rounded-2xl hover:bg-slate-800 transition-colors border-b border-slate-800 last:border-0 group">
                           ...
                           <div className="px-4 py-1 rounded-lg bg-slate-800 group-hover:bg-teal-900/30 transition-colors">
                                <span className="text-teal-400 font-mono font-bold text-lg">{time}</span>
                           </div>
                       </div>
                   ))}
               </div>
               ...
               <div className="flex items-center justify-between bg-slate-900 border border-slate-800 p-4 rounded-2xl mb-4">
                   ...
                   <button 
                    ...
                    className={`text-xs font-bold px-3 py-1.5 rounded-lg transition-colors border ${
                        ...
                        ? 'bg-teal-900/30 border-teal-500/30 text-teal-400 hover:bg-teal-900/50' 
                       ...
                       {waStatus === 'DISCONNECTED' && (
                           <div className="text-center animate-fade-in z-10">
                               ...
                               <button 
                                ...
                                className="px-8 py-3 bg-teal-600 hover:bg-teal-500 text-white font-bold rounded-xl shadow-lg shadow-teal-500/20 transition-all transform hover:-translate-y-1"
                               >
                       ...
                       {waStatus === 'QR_READY' && qrCodeData && (
                           <div className="text-center animate-fade-in z-10">
                               ...
                               <button 
                                ...
                                className="px-6 py-3 bg-gradient-to-r from-green-600 to-teal-600 text-white font-bold rounded-xl shadow-lg transition-all hover:brightness-110 flex items-center gap-2 mx-auto"
                               >
                       ...
                       <div className="bg-slate-900 p-3 rounded-2xl border border-slate-800">
                           <div className="flex gap-2">
                               <input 
                                    ...
                                    className="flex-1 bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white outline-none focus:border-teal-500 text-xs disabled:opacity-50"
                                    onKeyDown={(e) => e.key === 'Enter' && handleBroadcast()}
                               />
                               <button 
                                onClick={handleBroadcast}
                                disabled={waStatus !== 'CONNECTED' || !broadcastMsg}
                                className="bg-teal-600 disabled:bg-slate-700 hover:bg-teal-500 text-white w-10 h-10 rounded-lg flex items-center justify-center transition-colors shadow-lg"
                               >
